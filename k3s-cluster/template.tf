resource "oci_core_instance_configuration" "k3s_agent_template" {
  ...
  display_name   = "Ubuntu 20.04 instance k3s agent configuration"
  ...
  instance_details {
    ...
    launch_details {
      ...
      display_name = "Ubuntu k3s agent template"

      metadata = {
        "ssh_authorized_keys" = file(var.PATH_TO_PUBLIC_KEY)
        "user_data"           = data.template_cloudinit_config.k3s_agent_tpl.rendered
      }
